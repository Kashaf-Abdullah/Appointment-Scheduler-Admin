...
const DeleteCalenderTime = () => {
  ...
  const handleDeleteSchedule = async (id) => {
    try {
      const response = await axios.delete(`${process.env.REACT_APP_API_BASE_URL}/api/schedules/${id}`);
  ...
  const handleDeleteTime = async (scheduleId, time) => {
    try {
      const response = await axios.delete(`${process.env.REACT_APP_API_BASE_URL}/api/schedules/${scheduleId}/time/${time}`);
      setMessage(response.data.message);
